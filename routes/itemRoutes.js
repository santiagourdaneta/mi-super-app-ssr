...
// Rate limiter for 'get all items' root route: max 100 requests per 15 minutes per IP
const apiGetAllLimiter = RateLimit({
  windowMs: 15 * 60 * 1000, // 15 minutes
  max: 100, // limit each IP to 100 requests per windowMs
  message: 'Too many requests from this IP, please try again later.',
});

router.get('/', csrfProtection, apiGetAllLimiter, (req, res) => {
